...
void setup(void) {
  ...
  vis = XGetVisualInfo(
      dpy, VisualScreenMask | VisualDepthMask | VisualClassMask, &tpl, &nvi);
  for (i = 0; i < nvi; i++) {
  ...
  attrs.event_mask = FocusChangeMask | KeyPressMask | StructureNotifyMask |
                     ButtonMotionMask | ButtonPressMask | ButtonReleaseMask;
  attrs.background_pixmap = None;
  attrs.colormap = cmap;

  win = XCreateWindow(dpy, root, wx, wy, ww, wh, 0, 32, InputOutput, visual,
                      CWBackPixmap | CWBorderPixel | CWBitGravity |
